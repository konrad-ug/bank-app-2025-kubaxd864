...
def create_account(context, name, last_name, pesel):
    ...
    create_resp = requests.post(URL + "/api/create_account", json = json_body)
...
def is_account_count_equal_to(context, count):
    ...
    assert response.json()["length"] == int(count)
...
def incoming_transfer(context, amount, pesel):
    body = {"type": "incoming", "amount": float(amount)}
    resp = requests.post(URL + f"/api/account/{pesel}/transfer", json=body)
    assert resp.status_code == 200

@when('I make an outgoing transfer of "{amount}" from account with pesel "{pesel}"')
def outgoing_transfer(context, amount, pesel):
    body = {"type": "outgoing", "amount": float(amount)}
    resp = requests.post(URL + f"/api/account/{pesel}/transfer", json=body)
    if resp.status_code != 200:
        context.transfer_error = resp.status_code
    else:
        context.transfer_error = None
...
def balance_equals_to(context, pesel, balance):
    ...
    assert response.status_code == 200
    json_data = response.json()
    assert float(json_data["balance"]) == float(balance)

@then('Transfer fails with status code "{status_code}"')
def transfer_failed(context, status_code):
    assert context.transfer_error == int(status_code)